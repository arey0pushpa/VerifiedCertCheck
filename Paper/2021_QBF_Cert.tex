...
\usepackage{tikz}
\usetikzlibrary{tikzmark,decorations.pathreplacing,arrows,shapes,positioning,shadows,trees,shapes.gates.logic.US,arrows.meta,shapes,automata,petri,calc,fit}
...
%
%\institute{University of Manchester, United Kingdom \and JKU, Linz, Austria \\\email{peter.lammich@manchester.ac.uk, ankit.shukla@jku.at}}
%
...
\keywords{QBFs \and  Theorem proving \and Isabelle}
\end{abstract}

\section{Introduction}
\label{sec:intro}
%Quantified Boolean formulas (QBFs) is an extension of propositional formulas with existential and universal quantifiers over propositional variables. The support for quantifiers allows QBFs to represent problems more succinctly, for example the QBF encoding of Bounded model checking (BMC) \cite{DershowitzHK05} can be exponentially more compact than the corresponding propositional one. However, this comes with a price, QBFs are much more difficult to solve than propositional formulas. The decision problem of propositional satisfiability (SAT) is NP-complete \cite{Cook71}, whereas the decision problem of QBFs (QSAT) is the prototypical PSPACE-problem \cite{StockmeyerM73}. This makes QBFs a powerful host language for many application domains in automated reasoning and artificial intelligence \cite{ShuklaBPS19}, including planning \cite{AnsoteguiGS05, EglyKLP17, Cashmore2012planning, Gasquet18}, verification \cite{MillerSB13}, and synthesis \cite{BloemKS14, BloemEKKL14, Gange2014synthesizing}. 
...
%So, to construct the counter-examples, a QBF solver in addition to bare yes/no answers must provide \textit{certificates}. Certificates are used to validate the result of a solver as a witness for the decision. This increases the confidence in the correctness of the solver's result.
% to increase the confidence in their result and to construct the counter-examples. 
%Additionally, a certificate increases the confidence in the correctness of the solver's result.
...
%and enable reasoning tasks that require a witness.
...
%We address the above two bottlenecks by a formally verified certificate extraction approach to generate Skolem functions. This ensures complete confidence in the result of the solver where certificate is produced with a marginal overhead to the solving time.  It eliminates the use of the explicit validation process (SAT solver call). We show the effectiveness of our approach in the resolution-based certificate extraction \cite{NiemetzPLSB12}; our approach can be adopted to other QBF solving techniques \cite{RabeT15}.
...
%Reason? Maybe: A clause addition step can introduce new variables. XXX
...
\begin{figure}[t]
	\centering
	\begin{tikzpicture}[auto, node distance=1.8cm,>=latex', font = \sffamily, every text node part/.style={align=center},    %title/.style={font=\fontsize{6}{6}\color{black!50}\ttfamily}, typetag/.style={rectangle, draw=black!50},
	background/.style={rectangle, rounded corners, fill=black!0, draw=black!15, inner sep=4mm}]
	
	% \node (decomp) [title] { Decomposition };
	%\draw [draw=black!50] (decomp.north west) rectangle +(4cm, -6.7cm);
	\node [block, name=input] (controller) {DepQBF \\ (QBF
		Solving)};
	\node [block, name=input,below of=controller] (prop1) {QRPCheck \\ (Proof Extraction,
		Checking)};
	\node [block, label={[name=sub1]}, below of=prop1] (prop) {QRPCert \\ (Certificate
		Extraction)};  
	
	\node [sum, above of=controller, node distance=1.5cm] (system) {};
	\draw [->,semithick] (system) -- node[name=w] {\textsf{QBF $\FFF$}} (controller);
	\node [block, right of=prop, node distance=5.8cm] (skolem) {CertCheck \\ (Skolemizations)};
	\node [output, right of=controller, node distance=3.2cm] (unsat) {};    
%	\node [block1, left of=controller, node distance=3.8cm] (system) {QRATtrim};
%	\node [block1, right of=controller, node distance=4cm] (system2) {sk-extract};
%	\node [below of=system2, node distance=1.8cm] (skolem1) {$f^{\prime}$};
	\node [block4, name=input, below of=skolem, node distance=2cm] (prop4) {PicoSAT \\ (Certificate Validaion)};  
%	\node [right of=prop4, node distance=1.8cm] (skolemf) {$f$};
%	\node [below of=system, node distance=1.8cm] (skolemf2) {$f$};
	
	\node [ below of=prop, node distance=1.8cm] (cert1) {$f$};
	\draw [->,semithick] (prop) -- node[name=w] {Verified Certificate} (cert1);
	
	
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (controller) -- node[name=u] {\textsf{SAT?}} (system);
	
	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (prop1) -- node[right] {QRP Proof} (prop);
	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (controller) -- node[name=u] {trace QRP} (prop1);
	\draw [->,decorate, dotted, decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (prop) -- node[name=u] {\textsf{Certificate}} (skolem);
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (system2) -- node[name=u] {\textsf{Skolem}} (skolem1);
	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (skolem) -- node[name=u] {Prop Fml} (prop4);
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (skolem1) -- node[name=u] {} (prop4);
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (prop4) -- node[name=u] {} (skolemf);  
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (system) -- node[name=u] {} (skolemf2);  
%	\draw [->,thick,decorate,decoration={amplitude=.4mm,segment length=2mm,post length=1mm}] (controller) -- node[name=u] {QRAT trace} (system2);

%\node (dep) at (1cm, 0) [title] { Dependency };
%\draw [draw=black!50] (dep.north west) rectangle ($(dep.north east) - (2, 5cm)$);
\node[draw=none,fill=none,rectangle, rounded corners, above=0.0cm of prop1,xshift=-4.1cm,anchor=south west]
(RefProof){Verfied by Isabelle};

  \node[draw,fit=(RefProof)  (prop) (prop1), dotted] (LeftScope){};

\node[draw=none,fill=none,rectangle, rounded corners, above=0.2cm of skolem,xshift=-1.1cm,anchor=south west]
(RefProof1){Not required};

\node[draw,fit=(skolem)(prop4)(RefProof1), dotted] (LeftScope){};


	\end{tikzpicture}
	\caption{Veri-$\qrpcert$ tool chain. Formally verify the certificate extraction and avoid the expensive certificate validation process (SAT solver all).}
\end{figure}
