...
static inline double get_wc_time() {
  struct timeval t;
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
      ...
      if (strcmp(argv[i], "-P") == 0) options.print_proof_only = 1;
      ...
      if (++i < argc) {
        if (strcmp(argv[i], "qrp") == 0)
          ...
          QRPC_ABORT(1, "unknown output format for option %s: %s", argv[i - 1],
                     argv[i]);
      }
      ...
      QRPC_ABORT(fstat(in_qdimacs_fd, &file_info) == -1, "failed to fstat '%s'",
                 filename_qdimacs);
      in_qdimacs_size = file_info.st_size;
      QRPC_ABORT((in_qdimacs = (char *)mmap(0, in_qdimacs_size, PROT_READ,
                                            MAP_PRIVATE | MAP_NORESERVE,
                                            in_qdimacs_fd, 0)) == MAP_FAILED,
      ...
      QRPC_ABORT(fstat(in_qrp_fd, &file_info) == -1, "failed to fstat '%s'",
                 filename_qrp);
      in_qrp_size = file_info.st_size;
      QRPC_ABORT((in_qrp = (char *)mmap(0, in_qrp_size, PROT_READ,
                                        MAP_PRIVATE | MAP_NORESERVE, in_qrp_fd,
                                        0)) == MAP_FAILED,
                 "failed to mmap original formula");
      close(in_qrp_fd);
    }
...
#ifndef NPICOSAT
  if (options.print_proof_only) options.check_icubes = 0;
#endif
...
#ifndef NPICOSAT
    if (options.check_icubes) statistics.num_realloc_stack_initial_cubes += 1;
#endif
...
static unsigned char get_non_eof_ch(char *in, long *pos, long in_size) {
  char ch;
...
static void read_bin_lit(char *lits) {
...
static void read_ascii_lit(char *lits) {
  do {
    ch = lits[pos++];
  } while (isspace(ch));
...
static int _check(StepId id) {
  ...
  if (options.print_statistics) statistics.num_step_ref_total += 1;

  if (steps[id].visited) return SUCCESS;
  ...
  {
    if (options.verbosity > 1) fprintf(stderr, ": input clause/cube\n");
#ifndef NPICOSAT
    if (options.check_icubes) {
      if (options.print_statistics &&
          (unsigned int)(initial_cubes.sp - initial_cubes.bp) >=
              initial_cubes.size)
  ...
  if (options.print_proof_only) goto CHECK_ANTECEDENTS;
  ...
  for (i = 0; i < steps[id].num_ants; i++) {
    ...
    for (j = 0, pos = 0; j < steps[steps[id].ants[i]].num_lits; j++) {
      ...
      if (vars[vid].type == ptype && (!neg || GET_MARK(vid) != NEG) &&
          (neg || GET_MARK(vid) != POS)) {
  ...
  for (tmp = res_lits.bp; steps[id].num_ants == 2 && tmp < res_lits.sp; tmp++) {
    ...
    assert((vid == pivot && GET_MARK(vid) == NONE) || GET_MARK(vid) == POS ||
           GET_MARK(vid) == NEG);
  ...
  if (options.verbosity > 1) {
    ...
    for (tmp = res_lits.bp; tmp < res_lits.sp; tmp++) PRINT_MARK(*tmp);
    fprintf(stderr, ")\n");
  }
  ...
  for (i = 0, pos = 0; i < steps[id].num_lits; i++) {
    ...
    if (GET_MARK(vid) == DCPN) SET_MARK(vid, 0, neg ? DCN : DCP);
    ...
    if ((neg && GET_MARK(vid) != NEG && GET_MARK(vid) != DCN) ||
      ...
      if (options.verbosity > 1) {
        ...
        fprintf(stderr, ")\n");
        fprintf(stderr, "  ERROR: var %d is [%s], but [", vars[c_vid].idx,
                c_neg ? "NEG" : "POS");
    ...
    if (GET_MARK(vid) == POS || GET_MARK(vid) == NEG) num_lits_ndc += 1;
  }
  ...
  if (num_lits_ndc != num_lits_min) {
    if (options.verbosity > 1) {
      ...
      fprintf(stderr, ")\n");
      fprintf(stderr, "  ERROR: number of POS/NEG literals (%d)", num_lits_ndc);
  ...
  if (options.verbosity > 1) fprintf(stderr, "  ok\n");
...
#ifndef NPICOSAT
  if (options.check_icubes) STACK_INIT(initial_cubes, STACK_SIZE);
#endif
  return _check(start_sidx);
}

#ifndef NPICOSAT
static unsigned int simplify_qcnf(StepId id) {
  ...
  for (i = 0, pos = 0; i < steps[id].num_lits; i++) {
    ...
    for (j = 0; j < num_occs; j++) {
      if (clauses[occs_tmp[j]].is_sat) continue;
...
static int check_initial_cubes(void) {
  ...
  for (tmp = initial_cubes.bp; tmp != initial_cubes.sp; tmp++) {
    ...
    if (simplify_qcnf(*tmp) == num_clauses) {
      if (options.verbosity >= 1) fprintf(stderr, "OK (cover set)\n");
      if (options.print_statistics) statistics.num_steps_initial_full += 1;
    ...
    for (i = 0; i < num_clauses; i++) {
      ...
      if (options.verbosity > 1) fprintf(stderr, "  added clauses[%d]: (", i);
...
static void print_bin_num(int num, int is_literal) {
  unsigned char ch;
  unsigned int x = num;

  if (is_literal) x = num < 0 ? (-x << 1) | 1 : x << 1;
...
static void print_ascii_num(int num, int is_literal) {
...
static void print_proof(void) {
  ...
  for (i = num_vars; vars[i].s_level == 0 && vars[i].type == QTYPE_EXISTS;
       i--) {
    if (i == num_vars) {
...
static void print_statistics(void) {
  ...
  STATS("number of literals in %s                %11llu\n\n",
        qrp_type == QRPTYPE_UNSAT ? "original clauses: " : "initial cubes:    ",
  ...
  STATS(
      "number of references per step:                          %11.2f\n",
      (double)statistics.num_step_ref_total / statistics.num_steps_total_core);
  STATS_HR;
  STATS("TIME\n\n");
  STATS("total                                                   %8.2f ms\n",
...
static int parse_ascii_num(void) {
  ...
  if (p.pos == p.in_size) ch = EOF;
  return num;
}

static int parse_ascii_lit(void) {
  if ((neg = ch == MINUS)) SKIP_SPACE_DO_WHILE;
  parse_ascii_num();
  return num;
}

/* Note: Preamble is in ASCII in any case (QRP, binQRP, QDIMACS). */
static void parse_preamble(void) {
  ...
  if ((unsigned int)p.read_number() >= size_vidx2id) {
    ...
    if (vars == NULL) {
      ...
      size_vars = num + 1;
      if (options.print_statistics) statistics.num_realloc_vars += 1;
    }
    if (options.print_statistics) statistics.num_realloc_vidx2id += 1;
  }
  ...
  if (p.is_qrp) {
    ...
    if (options.print_statistics) statistics.num_realloc_clauses += 1;
  }
#endif
}

static void parse_scopes(void) {
  ...
  for (;;) {
    ...
    if (num_scopes == 0 && scope_type == QTYPE_FORALL) num_scopes += 1;
...
static void parse_literals(unsigned int id) {
  char eol = p.is_qrp ? QRP_EOL : QDIMACS_EOL;

#ifndef NPICOSAT
  if (!p.is_qrp) memset(mark_occs, 0, (num_vars + 1) * sizeof(byte_t));
#endif
  ...
  for (;;) {
    ...
    if (vidx2id[num] == 0) /* free var */
    {
      ...
      if (options.print_statistics) statistics.num_free_vars += 1;
    }
...
static void parse_qrp(char *filename_qrp, char *in_qrp, long in_qrp_size) {
  char eol = QRP_EOL;
  ...
  if (options.print_statistics) statistics.num_realloc_mark_occs += 1;
  ...
  for (;;) {
    ...
    if ((sidx = num) >= size_sidx2id) {
      ...
      if (sidx > max_sidx) max_sidx = sidx;

      if (options.print_statistics) statistics.num_realloc_sidx2id += 1;
    }
    if (sidx2id[sidx] == 0) {
      if ((sidx2id[sidx] = ++(num_steps)) >= size_steps) /* increase by half */
      {
        ...
        if (options.print_statistics) statistics.num_realloc_steps += 1;
      }
    }
    ...
    for (;;) {
      ...
      if ((unsigned int)num >= size_sidx2id) {
        ...
        QRPC_REALLOC(sidx2id, size_sidx2id, new_size);
        size_sidx2id = new_size;
        if ((unsigned int)num > max_sidx) max_sidx = sidx;
    ...
    if (steps[sidx2id[sidx]].num_ants == 0) steps[sidx2id[sidx]].is_initial = 1;

    if (options.print_statistics) {
      switch (steps[sidx2id[sidx]].num_ants) {
        case 0:
          statistics.num_steps_initial += 1;
          break;
        case 1:
          statistics.num_steps_red += 1;
          break;
        default:
          statistics.num_steps_res += 1;
      }
    }
...
#ifndef NPICOSAT
static void parse_qdimacs(char *filename_qdimacs, char *in_qdimacs,
                          long in_qdimacs_size) {
  ...
  if (options.print_statistics) statistics.num_realloc_occurrences += 1;
...
static void cleanup(void) {
  if (vidx2id != NULL) free(vidx2id);
  if (sidx2id != NULL) free(sidx2id);
  if (vars != NULL) free(vars);
  if (steps != NULL) free(steps);
  if (mark_occs != NULL) free(mark_occs);
#ifndef NPICOSAT
  if (occurrences != NULL) free(occurrences);
  ...
  if (in_qrp != NULL) munmap(in_qrp, in_qrp_size);
#ifndef NPICOSAT
  if (in_qdimacs != NULL) munmap(in_qdimacs, in_qdimacs_size);
#endif
}
